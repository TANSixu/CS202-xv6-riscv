...
//lab3 for thread, not free shared resource
static void
...
// For lab3, modified from allocproc
static struct proc*
...
found:
  p->pid = allocpid();
  p->thread_id = parent->thread_cnt + 1;
  parent->thread_cnt++;
  p->state = USED;
  ...
  //map trapframe under the parent's
